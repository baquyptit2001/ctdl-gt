...
void    dothi::dfstree(int u){  //Duyet dfs tren cay
    chuaxet[u]=false;
    for(int i=1; i<=n; i++)
...
void    dothi::prim(int s){
    ...
    bubblesort();
    while(net!=n-1){
        for(int i=1; i<=ne; i++){   //moi lan se lay 1 canh be nhat co 1 dinh thuoc VH, 1 dinh thuoc V
            init();
            ...
            if(chuaxet[dau]!=chuaxet[cuoi]){    //canh i thoa man
                net++;
                tree[net].dau=dau;
                tree[net].cuoi=cuoi;
                dH+=graph[i].h;
